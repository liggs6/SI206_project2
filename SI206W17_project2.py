...
def find_urls(x):
	u_lst = []
	reg = r"https?:\/\/[0-9A-Za-z]+(?:\.+[0-9A-Za-z]{1,})+"
	url = re.findall(reg, x)
	for x in url:
		u_lst.append(x)
	return u_lst
## For example: 
...
# find_urls("the internet is awesome #worldwideweb")
...
def get_umsi_data():
	url = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page="## Start with this page: https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All  
	umsi_lst = []
	if "umsi_directory_data" in CACHE_DICTION:## The function should check if there is any cached data for the UMSI directory in your cached file -- if so, return it, and if not, the function should access each page of the directory, get the HTML associated with it, append that HTML string to a list. The function should cache (save) that list when it is accumulated.
		html_strings = CACHE_DICTION["umsi_directory_data"]
...
## End with this page: https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page=11 
			base_url = url + str(x)
			data_collection = requests.get(url, headers={'User-Agent': 'SI_CLASS'})#### requests.get(base_url, headers={'User-Agent': 'SI_CLASS'}) 
			umsi_lst.append(data_collection.text)
...
# htmls = get_umsi_data()
umsi_titles = {} ## whose keys are UMSI people's names, and whose associated values are those people's titles, e.g. "PhD student" or "Associate Professor of Information"...

for x in range(12):
	soup = BeautifulSoup(CACHE_DICTION["umsi_directory_data"][x], "html.parser")
	people = soup.find_all("div",{"class":"views-row"})

	for x in people:
		name = x.find("div",{"property":"dc:title"}).text
		title = x.find("div", {"class":"field-name-field-person-titles"}).text
...
def get_five_tweets(x):
	unique = "twitter_{}".format(x)
	if unique in CACHE_DICTION:
		response = CACHE_DICTION[unique]

	else:
		response = api.search(q=any_string)
		CACHE_DICTION[unique] = response
		dict = open(CACHE_FNAME,'w')
		dict.write(json.dumps(CACHE_DICTION))
		dict.close()

	top_tweets = []

	for x in response["statuses"][:5]:
		top_tweets.append(x['text'])
