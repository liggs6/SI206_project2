...
def parse_counted_words(tup):
    find = re.findall(r"(\d+)\s(\W[A-Za-z]+|[A-Za-z]+)", tup)
    if find:
        return (find[-1])
    else:
        return None
...
for x in file_lines:
    python_paths = re.findall(r"SI206\/(\S+.py)",x)
...
for x in file_lines:
    microsoft = re.findall(r"[0-9](\.xlsx|.docx)", x)
    if len(microsoft)>0:
