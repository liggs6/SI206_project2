...
search_input = input("Enter something you would like to search for on Twitter: ")

def tweets(search_input):
	if search_input in CACHE_DICTION:

		response = CACHE_DICTION[search_input]
	else:

		response = api.search(q= search_input)
		CACHE_DICTION[search_input] = response
		new_search = open(CACHE_FNAME, 'w')
		new_search.write(json.dumps(CACHE_DICTION))
		new_search.close
	return response['statuses']

for tweet in tweets(search_input)[0:3]:
	print("\n")
